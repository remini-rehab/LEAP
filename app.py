...
def explain_latest(row):
    ...
    if pd.notna(row["fail_7d"]) and row["fail_7d"] >= 3:
        reasons.append(f"최근 7일 야간 회복 실패 {int(row['fail_7d'])}회")

    if pd.notna(row["am_range_7d"]) and row["am_range_7d"] >= 0.004:
        reasons.append(f"7일 오전 변동폭 증가 {row['am_range_7d']:.4f}")

    if pd.notna(row["pm_range_7d"]) and row["pm_range_7d"] >= 0.006:
        reasons.append(f"7일 오후 변동폭 증가 {row['pm_range_7d']:.4f}")

    if pd.notna(row["cv_7d"]) and row["cv_7d"] >= 1.0:
        reasons.append(f"7일 비율 변동성 증가 {row['cv_7d']:.2f}%")
...
def create_figure(analyzed_df):
    ...
    ax1.set_title("상지 동태 분석 (★: Ratio ≥ 1.02)", fontsize=14, fontweight="bold")
    ax1.grid(True, linestyle="--", alpha=0.35)
    ax1.legend(loc="upper left", bbox_to_anchor=(1, 1))

    ax2.plot(analyzed_df["검사일시"], analyzed_df["leg_drift"], marker="o", linewidth=1.8, label="하지 이탈")
    ax2.plot(analyzed_df["검사일시"], analyzed_df["trunk_drift"], marker="s", linewidth=1.8, label="체간 이탈")
    ax2.axhline(0, color="black", linewidth=1)
    ax2.set_title("전신 기준선 이탈 추이 (하지/체간)", fontsize=12)
...
def build_pdf(patient_name, report_date, latest_row, fig):
    ...
    pdf.cell(0, 7, f"- 최근 3일 ratio 경고 횟수: {safe_fmt(latest_row['warn_ratio_3d'], '.0f')}", ln=1)

    pdf.cell(0, 7, f"- 7일 회복 실패 횟수: {safe_fmt(latest_row['fail_7d'], '.0f')}", ln=1)
    pdf.cell(0, 7, f"- 7일 오전 변동폭: {safe_fmt(latest_row['am_range_7d'])}", ln=1)
    pdf.cell(0, 7, f"- 7일 오후 변동폭: {safe_fmt(latest_row['pm_range_7d'])}", ln=1)
    pdf.cell(0, 7, f"- 7일 비율 변동계수(CV): {safe_fmt(latest_row['cv_7d'], '.2f')}%", ln=1)

    pdf.cell(0, 7, f"- 하지 drift: {safe_fmt(latest_row['leg_drift'])}", ln=1)
    pdf.cell(0, 7, f"- 체간 drift: {safe_fmt(latest_row['trunk_drift'])}", ln=1)
    pdf.ln(4)
            ...
            if mode == "👤 개별 환자 진료 (1:1 상담용)":
                ...
                with col2:
                    st.warning(
                        ...
                        f"- 3일 ratio 경고: {safe_fmt(latest_row['warn_ratio_3d'], '.0f')}회"
                    )

                with col3:
                    st.error(
                        f"**🔴 7일/전신 관점**\n\n"
                        f"- 7일 회복 실패: {safe_fmt(latest_row['fail_7d'], '.0f')}회\n"
                        f"- 7일 오전 변동폭: {safe_fmt(latest_row['am_range_7d'])}\n"
                        f"- 7일 오후 변동폭: {safe_fmt(latest_row['pm_range_7d'])}\n"
                        f"- 7일 비율 CV: {safe_fmt(latest_row['cv_7d'], '.2f')}%\n"
                        ...
                        for i, sheet in enumerate(sheet_names):
                            try:
                                ...
                                summary_data.append({
                                    ...
                                    "3일 실패": safe_fmt(latest_row["recovery_fail_3d"], ".0f"),
                                    "7일 실패": safe_fmt(latest_row["fail_7d"], ".0f"),
                                    "7일 CV(%)": safe_fmt(latest_row["cv_7d"], ".2f"),
